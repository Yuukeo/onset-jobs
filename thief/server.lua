...
local thiefNpcCached = {}
local playerThief = {}

AddEvent("OnPackageStart", function()
    for k,v in pairs(thiefNpc) do
        thiefNpc[k].npc = CreateNPC(thiefNpc[k].location[1], thiefNpc[k].location[2], thiefNpc[k].location[3], thiefNpc[k].location[4])
        CreateText3D(_("thief_job").."\n".._("press_e"), 18, thiefNpc[k].location[1], thiefNpc[k].location[2], thiefNpc[k].location[3] + 120, 0, 0, 0)
        table.insert(thiefNpcCached, thiefNpc[k].npc)
    end
end)

AddEvent("OnPlayerQuit", function( player )
    if playerThief[player] ~= nil then
        playerThief[player] = nil
    end
end)

AddEvent("OnPlayerJoin", function(player)
    CallRemoteEvent(player, "SetupThief", thiefNpcCached)
end)

AddRemoteEvent("StartStopThief", function(player)
	if PlayerData[player].thief == 0 then
	...
	CallRemoteEvent(player, "MakeNotification", _("join_thief"), "linear-gradient(to right, #00b09b, #96c93d)")
	return
    end 
	...
	CallRemoteEvent(player, "MakeNotification", _("quit_thief"), "linear-gradient(to right, #00b09b, #96c93d)")
...
AddRemoteEvent("OpenThiefMenu", function(player)
    if PlayerData[player].job == "thief" then
        CallRemoteEvent(player, "ThiefMenu")
    end
end)

AddRemoteEvent("OpenThiefFineMenu", function(player)
    if PlayerData[player].job == "thief" then
	...
	for k,v in pairs(playersIds) do
	    ...
	    if(GetDistance3D(x, y, z, _x, _y, _z) < 500 and player ~= v and PlayerData[k].job ~= "thief") then
		playersNames[tostring(k)] = GetPlayerName(k)
	    end
	end
	CallRemoteEvent(player, "OpenThiefFineMenu", playersNames)
    end
end)

function GetNearestThief(player)
	local x, y, z = GetPlayerLocation(player)
...
function GetUniformServer(player)
    CallRemoteEvent(player, "ChangeUniformClient", player, PlayerData[player].clothing_thief[1], 0)
    CallRemoteEvent(player, "ChangeUniformClient", player, PlayerData[player].clothing_thief[3], 1)
    CallRemoteEvent(player, "ChangeUniformClient", player, PlayerData[player].clothing_thief[4], 4)
    CallRemoteEvent(player, "ChangeUniformClient", player, PlayerData[player].clothing_thief[5], 5)
...
function ChangeUniformOtherPlayerServer(player, otherplayer)
    ...
    if PlayerData[otherplayer].clothing_thief == nil then
	return
    end
    CallRemoteEvent(player, "ChangeUniformClient", otherplayer, PlayerData[otherplayer].clothing_thief[1], 0)
    CallRemoteEvent(player, "ChangeUniformClient", otherplayer, PlayerData[otherplayer].clothing_thief[3], 1)
    CallRemoteEvent(player, "ChangeUniformClient", otherplayer, PlayerData[otherplayer].clothing_thief[4], 4)
    CallRemoteEvent(player, "ChangeUniformClient", otherplayer, PlayerData[otherplayer].clothing_thief[5], 5)
end
...
function GetPatrolCar(player)
    local nearestThief = GetNearestThief(player)
	...
	for k,v in pairs(GetAllVehicles()) do
	    ...
	    local dist2 = GetDistance3D(thiefNpc[nearestThief].spawn[1], thiefNpc[nearestThief].spawn[2], thiefNpc[nearestThief].spawn[3], x, y, z)
	    if dist2 < 500.0 then
	...
	if isSpawnable then
	    local vehicle = CreateVehicle(3, thiefNpc[nearestThief].spawn[1], thiefNpc[nearestThief].spawn[2], thiefNpc[nearestThief].spawn[3], thiefNpc[nearestThief].spawn[4])
